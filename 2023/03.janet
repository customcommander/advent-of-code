...
(def schema-grammar
  (peg/compile
   ~{:main (some :line)
     :line (* (some (+ :num :dot+ :sym)) :eol)
     :num (/ (* :pos (number :d+) :pos)
             ,(fn [[a b] n [x y]]
                {:number n
                 :top [(dec a) (dec b)]
                 :bottom [(inc x) y]}))
     :dot "."
     :dot+ (some :dot)     
     :sym (/ (* :pos (if-not :eol 1)) ,| {:symbol $0}) 
     :pos (group (* (line) (column)))
     :eol (+ "\n" -1)}))

(defn answer [text]
  (let [matches (peg/match schema-grammar text)
        numbers (filter | (has-key? $0 :number) matches)
        symbols (filter | (has-key? $0 :symbol) matches)]
    (sum
     (mapcat (fn [{:symbol loc}]
               (map | (if (adjacent? ($0 :top) ($0 :bottom) loc)
                        ($0 :number)
                        0) numbers))
             symbols))))
