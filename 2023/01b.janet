(defn calibration-value [str]
  (let [len (length str)]
    (cond
      (= len 1) (parse (string str str))
      (= len 2) (parse str)
      (parse (string (string/slice str 0 1)
                     (string/slice str -2))))))

(def calibration-grammar
  (peg/compile
   ~{:main (some (/ (% :line) ,calibration-value))
     :line (* (some (+ :digit :a)) (+ "\n" -1))
...
(defn answer [text]
  (sum (peg/match calibration-grammar text)))
