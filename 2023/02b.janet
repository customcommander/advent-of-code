(def games-log-grammar
  (peg/compile
   ~{:main (some :game)
     ...
     :game (/ (* "Game " (number :d+) ": " (group (some :set)))
              ,(fn [id sets]
                 {:game id
                  :smallest-bag {:red   (max ;(map | (or ($ :red  ) 0) sets))
                                 :green (max ;(map | (or ($ :green) 0) sets))
                                 :blue  (max ;(map | (or ($ :blue ) 0) sets))}}))

     :set (/ (group (* :draw (any (* ", " :draw)) :end-of-set))
             ,(fn [draws]
                (merge ;draws)))
...
(defn answer [text]
  (let [games (peg/match games-log-grammar text)]
    (reduce (fn [tot {:game id :smallest-bag {:red r :green g :blue b}}]
              (+ tot (* (if (= r 0) 1 r)
                        (if (= g 0) 1 g)
                        (if (= b 0) 1 b))))
            0
